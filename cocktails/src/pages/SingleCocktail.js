...
const SingleCocktail = () => {
  //useParams hook so we can access params
  const {id} = useParams()
    ...
    async function getCocktail(){
      try{
        ...
        const {strDrink:name,strDrinkThumb:image,
          ...
          const newCocktails ={name,image,info,category,
  ...
  if(!cocktail){
    return <h2 className='section-title'>No cocktail mate</h2>
  }
  const {name,image,category,info,glass,instructions,ingredients} = cocktail
  return (
    <section className='section cocktail-secion'>
